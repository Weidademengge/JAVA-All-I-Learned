...
![[20210201232459724.png]]


## 1.AbstractApplicationContext是干嘛的
AbstractApplicationContext主要实现了上下文初始化功能，包括：
- 定义了初始化BeanFactory
- 初始化了一些BeanFactoryPostProcessors
- 初始化MessageSource（国际化）
- 事件广播其（ApplicationEventMulticaster）

## 2.AbstractApplicationContext实现的接口和继承的类都是什么
抽象类 继承了**DefaultResourceLoader默认资源解析类**并且实现了ConfigurableApplicationContext

`ApplicationContext`是一个比BeanFactory更具丰富IOC容器接口，而`AbstractApplicationContext`是其的一个抽象实现，定义和实现了一系列的初始化流程。

`ApplicationContext`该接口具有三个常用的实现类：
...
在我们定义一个IOC容器时，debug会进入`AbstractApplicationContext`，这里我们看一下源码，看一下AbstractApplicationContext中都有哪些属性和方法。太多了，慢慢看。
```java
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader  
   ...
   /**  
    * lifecycleProcessor类的名字，如果没有对应实例，则会默认DefaultLifecycleProcessor
    */  
   ...
   static {  
	  ...
	  //说人话就是webLogic热部署时，会重新生成一个ClassLoader类加载器，然后报错：ClassCastException
   ...
   /** objectUtils工具类给与对应实例唯一ID，名字@hashcode形式
   ...
   /**  
    * Create a new AbstractApplicationContext with no parent.    */   public AbstractApplicationContext() {  
   ...
   /**  
    * Set the unique id of this application context.    * <p>Default is the object id of the context instance, or the name  
    * of the context bean if the context is itself defined as a bean.    * @param id the unique id of the context  
    */   @Override  
    ...
    * (that is, this context is the root of the context hierarchy).    */   @Override  
   @Nullable  
    ...
    * form, allowing for further customization.    * <p>If none specified, a default environment will be initialized via  
   ...
   /**  
    * Return the timestamp (ms) when this context was first loaded.    */   @Override  
   public long getStartupDate() {  
      return this.startupDate;  
   }  
  
   /**  
    * Publish the given event to all listeners.    * <p>Note: Listeners get initialized after the MessageSource, to be able  
    * to access it within listener implementations. Thus, MessageSource    * implementations cannot publish events.    * @param event the event to publish (may be application-specific or a  
    * standard framework event)    */   @Override  
   ...
   /**  
    * Publish the given event to all listeners.    * <p>Note: Listeners get initialized after the MessageSource, to be able  
    * to access it within listener implementations. Thus, MessageSource    * implementations cannot publish events.    * @param event the event to publish (may be an {@link ApplicationEvent}  
   ...
   /**  
    * Publish the given event to all listeners.    * @param event the event to publish (may be an {@link ApplicationEvent}  
    * or a payload object to be turned into a {@link PayloadApplicationEvent})  
    ...
    * etc in certain AbstractApplicationContext subclasses.    * @param beanFactory the bean factory used by the application context  
